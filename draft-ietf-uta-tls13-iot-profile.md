...
Note that in environments where protection against malware is essential (e.g., enterprise networks, industrial IoT deployments), using ECH may not be an option because network administrators want to be able to filter the DNS traffic generated by IoT devices in order to prevent access to potentially malicious content.
